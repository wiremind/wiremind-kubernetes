...
def test_stop_pods_priority(mocker: MockerFixture) -> None:
    ...
    expected_calls = [
        mock.call._stop_deployments({"first": 17}),
        mock.call._stop_deployments({"second": 17}),
        mock.call._stop_deployments({"last": 42}),
    ]
...
def test_stop_deployments_correctly_wait(mocker: MockerFixture) -> None:
    ...
    expected_calls = [
        mock.call._are_deployments_stopped(deployment_dict),
        mock.call._are_deployments_stopped(deployment_dict),
        mock.call._are_deployments_stopped(deployment_dict),
    ]
